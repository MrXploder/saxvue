const isColor = (color: string) => {
  ...
  return svColors.includes(color);
};

const setVar = (propertyName: string, value: string, el: HTMLElement | null) => {
  if (!el) {
    document.documentElement.style.setProperty(`--sv-${propertyName}`, value);
  } else {
    if (el.nodeName !== '#comment') {
      el.style.setProperty(`--sv-${propertyName}`, value);
    }
  }
};

const setColor = (colorName: string, color: string, el: HTMLElement | null, addClass?: boolean) => {
  ...
  if (color == 'dark' && el) {
    if (addClass) {
      el.classList.add('sv-component-dark');
    }
  }
...
const insertBody = (element: HTMLElement, parent: HTMLElement | null) => {
  const target = parent ? parent : document.body;
  target.insertBefore(element, target.lastChild);
};

const removeBody = (element: HTMLElement, parent: HTMLElement | null) => {
  const target = parent ? parent : document.body;
